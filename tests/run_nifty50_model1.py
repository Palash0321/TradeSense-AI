...
FEATURE_FILE = Path("data/nifty50_features/nifty50_t1_features.csv")

OUTPUT_DIR = Path("data/nifty50_model1")
...
FEATURES = NIFTY_FEATURES + BREADTH_FEATURES
...
def main():
    ...
    print("=" * 70)
    print("NIFTY 50 MODEL 1")
    print("NIFTY HISTORY + SIMPLE CONSTITUENT BREADTH")
    print("=" * 70)
    ...
    print(f"Usable Model 1 rows: {len(model_df)}")
    ...
    # --------------------------------------------------------
    # Baseline: unconditional majority
    # --------------------------------------------------------

    full_target = df[TARGET].dropna().astype(int)

    majority_class = int(
        full_target.mean() >= 0.50
    )
    ...
    # --------------------------------------------------------
    # Baseline: previous-day NIFTY direction
    # --------------------------------------------------------

    baseline_df = df[
        ["date", "nifty_return_1d", TARGET]
    ].dropna().copy()

    baseline_df["previous_day_prediction"] = (
        baseline_df["nifty_return_1d"] > 0
    ).astype(int)
    ...
    print(
        ...
        f"accuracy {majority_accuracy:.6f}, "
        f"balanced_accuracy "
    ...
    print(
        ...
        f"accuracy {previous_day_accuracy:.6f}, "
        f"balanced_accuracy "
    ...
    summary = {
        "model": "MODEL_1",
        "feature_group": (
            "NIFTY_HISTORY_PLUS_SIMPLE_CONSTITUENT_BREADTH"
        ),
        "feature_count": len(FEATURES),
        "nifty_feature_count": len(NIFTY_FEATURES),
        "breadth_feature_count": len(BREADTH_FEATURES),
        "walk_forward_observations": (
            overall_metrics["observations"]
        ),
        "actual_positive_rate": (
            predictions_df["actual"].mean()
        ),
        "predicted_positive_rate": (
            predictions_df["predicted"].mean()
        ),
