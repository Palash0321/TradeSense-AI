...
OUTPUT_DIR = BASE_DIR / "data" / "nifty50_model3"
...
FEATURES = NIFTY_FEATURES + CONSTITUENT_MOMENTUM_FEATURES
...
print("=" * 70)
print("NIFTY 50 MODEL 3")
print("NIFTY HISTORY + CONSTITUENT MOMENTUM")
print("=" * 70)
...
print(f"Usable Model 3 rows: {len(model_df):,}")

target_positive_rate = model_df[TARGET].mean()
...
y_baseline = baseline_df[TARGET].astype(int)
...
dates = model_df[DATE_COLUMN].to_numpy()
...
for i in range(MIN_TRAIN_ROWS, len(model_df)):
    ...
    y_test = int(test_df[TARGET].iloc[0])
    ...
    predictions.append(
        {
            "date": test_df[DATE_COLUMN].iloc[0],
            "actual": y_test,
...
for year, group in predictions_df.groupby(
    ...
    y_true_year = group["actual"].astype(int)
    y_pred_year = group["predicted"].astype(int)
    y_prob_year = group["probability_up"].astype(float)
